...
int main()
{
	...
	glm::mat4 camView = glm::lookAt(glm::vec3(-1, 2, 4), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));
	glm::mat4 camProj = glm::perspective(45.f, 1280.f / 720, 1.f, 100.f);
	...
	glm::vec4 lightColor = glm::vec4(1, 0, 0.75f, 1);
	...
	while (window.step())
	{
		...
		spearModel = glm::rotate(time, glm::vec3(0, 1, 0)) * glm::translate(glm::vec3(0, 1, 0) * time);

		clearFrameBuffer(gframe);
		tdraw(gpass, sphere, gframe, spearModel, camView, camProj, white, vertex_normals, white);
		tdraw(gpass, quad, gframe, wallModel, camView, camProj, white, vertex_normals, white);

		clearFrameBuffer(sframe);
		tdraw(spass, sphere, sframe, spearModel, lightView, lightProj);
